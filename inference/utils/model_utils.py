...
def build_emu3(model_path, tokenizer_path, local_rank=0, bad_model_path=None, vq_type="movqgan", vllm_engine=False):
    ...
    print(f"Model config vq_type: {getattr(model.config, 'vq_type', 'NOT_FOUND')}")
    print(f"Using vq_type: {vq_type}")
    ...
    vq_type = getattr(model.config, "vq_type", "movqgan")
    if vq_type == "movqgan":
        ...
        cfg = OmegaConf.load(osp.join(model.config.ibq_cache_dir, "config.yaml"))
        vq_model = IBQ(**cfg.model.init_args).to(model.device)
        ckpt = torch.load(osp.join(model.config.ibq_cache_dir, "model.ckpt"), map_location="cpu")["state_dict"]
    ...
    if vllm_engine:
        ...
        vllm_model = LLM(
            ...
            max_seq_len_to_capture=model_config.max_position_embeddings,
            # swap_space=80,
